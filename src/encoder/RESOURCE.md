...
- 当前语义：
  ...
  - 历史复用使用“预加载历史窗口 + 当前块持续建哈希”的增量 LZ77 生成策略
  - 内部自动落块粒度当前为 `128KB`
  - 当一序列块超出旧直写表示力时，会自动改走标准序列段格式
...
- 流式压缩器当前内部自动落块粒度为 `128KB`。
- 原因：
  - 当前一序列大值已经可以通过标准序列段格式编码
  - 同时修复了历史窗口位置 0 与空槽位哨兵冲突问题
  - 因此流式路径已经可以恢复到与普通块一致的 `128KB` 粒度
